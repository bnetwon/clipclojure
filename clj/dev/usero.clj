...
;           (println args))
;
(defn get-lnum [] (let [llist (sort (into #{} (take 7 (repeatedly #(let [ rint (rand-int 36)]
                                                                     (do (. Thread sleep (rint)) (inc rint)))))))

                        clist (count llist)]
                    (if (> clist 6) (let [jstr (clojure.string/join " " llist)](do (println jstr)(set-string jstr)) )(get-lnum))))
(defn get-lnum2 [] (let [llist (sort (into #{} (take 7 (repeatedly #(let [ rint (rand-int 36)]
                                                                      (do (. Thread sleep (rand-int 100)) (inc rint)))))))

                         clist (count llist)]
                     (if (> clist 6) (let [jstr (clojure.string/join " " llist)](do (println jstr)(set-string jstr)) )(get-lnum2))))

(defn get-lnum6 [] (let [llist (sort (into #{} (take 6 (repeatedly #(let [ rint (rand-int 44)]
                                                                      (do (. Thread sleep (rand-int 100)) (inc rint)))))))

                         clist (count llist)]
                     (if (> clist 5) (let [jstr (clojure.string/join " " llist)](do (println jstr)(set-string jstr)) )(get-lnum6))))
;
(def upper-case-set(set (map char (range 65 91))))
  ...
  (when s
    (some-> s
            (clojure.string/replace #"[-_\s]+(.)?"
                       (fn [[match c]]
                         (if c (clojure.string/upper-case c) ""))))))
;
  ...
  (when s
    (-> s
        (clojure.string/replace #"([A-Z][a-z]+)"
          (fn [[match c]]
            (if c (str "-" (clojure.string/lower-case c)) "")))
